...
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const loadProfile=async(userId:string)=>{const{data}=await supabase.from("profiles").select("*").eq("id",userId).maybeSingle();setProfile(data as SobrietyProfile|null); if (data) await syncV1CEWidget(data as SobrietyProfile)};
