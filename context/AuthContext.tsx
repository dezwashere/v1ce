...
const withTimeout=async<T>(promise:Promise<T>,ms=5000):Promise<T>=>await Promise.race([promise,new Promise<T>((_,reject)=>setTimeout(()=>reject(new Error("timeout")),ms))]);
export function AuthProvider({children}:{children:ReactNode}){
 ...
 const loadProfile=async(userId:string)=>{
  try{
   const{data}=await withTimeout(supabase.from("profiles").select("*").eq("id",userId).maybeSingle());
 ...
 useEffect(()=>{
  ...
  const initialize=async()=>{
   try{
    let{data:{session}}=await withTimeout(supabase.auth.getSession());
