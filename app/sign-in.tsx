...
export default function SignIn() {
  ...
  async function signInWithProvider(provider: "apple" | "google") {
    ...
    if (params.code) {
      ...
      if (exchangeError) return Alert.alert("Sign in failed", exchangeError.message);
    } else if (params.access_token && params.refresh_token) {
      const { error: sessionError } = await supabase.auth.setSession({
        access_token: params.access_token,
        refresh_token: params.refresh_token,
      });
      if (sessionError) return Alert.alert("Sign in failed", sessionError.message);
    } else {
      return Alert.alert("Sign in failed", "The provider did not return a valid session.");
    }
    router.replace("/");
  }
  ...
  async function submit() {
    ...
    if (mode === "signUp" && !result.data.session) {
      Alert.alert("Check your email", "We sent you a verification link. Tap it to finish creating your V1CE account.");
      return;
    }
    router.replace("/");
  }
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>{mode === "signIn" ? "Sign in to continue." : "Your recovery journey starts here."}</Text>
        ...
        <TextInput style={styles.input} placeholder="email" placeholderTextColor="#737373" autoCapitalize="none" autoCorrect={false} keyboardType="email-address" value={email} onChangeText={setEmail} />
...
const styles = StyleSheet.create({
  ...
  socialButton: { height: 54, alignItems: "center", justifyContent: "center", backgroundColor: "#FFFFFF", borderWidth: 2, borderColor: "#0A0A0A", marginBottom: 10 },
  ...
  forgotButton: { alignSelf: "flex-end", marginBottom: 6 },
  forgotText: { color: "#0A0A0A", fontSize: 14, fontWeight: "600" },
